...
BEGIN
  ...
  v_nights := EXTRACT(DAY FROM (p_date_fin_prevue - p_date_debut_prevue));
  ...
  INSERT INTO public.bookings (
    ...
    CASE WHEN p_is_immediate_checkin THEN 'CHECKED_IN' ELSE 'CONFIRMED' END
  ) RETURNING id INTO v_booking_id;

  RAISE NOTICE 'Réservation créée: %', v_booking_id;
  ...
  IF p_is_immediate_checkin THEN
    ...
    RAISE NOTICE 'Chambre % marquée comme OCCUPIED', v_room.numero;
  END IF;

  -- 3. CRÉER LA FACTURE
  v_invoice_number := 'FACT-' || TO_CHAR(NOW(), 'YYYYMMDD-HH24MISS') || '-' || LPAD(FLOOR(RANDOM() * 9000 + 1000)::TEXT, 4, '0');
  ...
  RAISE NOTICE 'Facture créée: % (statut: %)', v_invoice_number, v_invoice_status;
  ...
  IF v_total_payment_usd > 0 THEN
    INSERT INTO public.payments (
      ...
      v_booking_id, v_invoice_id, v_total_payment_usd, p_payment_method, NOW(), 'Paiement initial',
      p_initial_payment_usd, p_initial_payment_cdf, p_exchange_rate
    ) RETURNING id INTO v_payment_id;

    RAISE NOTICE 'Paiement initial créé: % (USD: %, CDF: % @ %)', v_total_payment_usd, p_initial_payment_usd, p_initial_payment_cdf, p_exchange_rate;
  END IF;

  -- 5. RETOURNER LE RÉSULTAT
  RETURN JSON_BUILD_OBJECT(
    'success', TRUE, 'booking_id', v_booking_id, 'invoice_id', v_invoice_id,
    'payment_id', v_payment_id, 'net_total', v_net_total, 'amount_paid', v_total_payment_usd,
    'balance_due', v_net_total - v_total_payment_usd
  );
