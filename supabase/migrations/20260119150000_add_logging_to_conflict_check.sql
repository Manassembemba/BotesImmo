-- Migration: Add extensive logging to booking conflict check
-- Date: 19 janvier 2026
-- Objectif: Ajouter des logs de débogage pour comprendre pourquoi un conflit est détecté de manière inattendue.
...
BEGIN
  -- Logic before conflict check is unchanged...
  ...
  IF p_is_immediate_checkin THEN v_forced_start_date := NOW();
  ELSE v_forced_start_date := (p_date_debut_prevue::date + interval '12 hours')::timestamptz;
  END IF;
  IF v_forced_end_date <= v_forced_start_date THEN RAISE EXCEPTION 'La date de fin doit être après la date de début'; END IF;

  -- ### DEBUGGING LOGS START ###
  RAISE NOTICE '--- Begin Conflict Check for Room ID % ---', p_room_id;
  RAISE NOTICE 'New booking interval to check: [%] to [%]', v_forced_start_date, v_forced_end_date;
  ...
  LOOP
    RAISE NOTICE 'Evaluating existing booking ID %: Start=[%], End=[%]',
      conflicting_booking.id,
      conflicting_booking.date_debut_prevue,
      conflicting_booking.date_fin_prevue;

    IF (conflicting_booking.date_debut_prevue, conflicting_booking.date_fin_prevue) OVERLAPS (v_forced_start_date, v_forced_end_date) THEN
        RAISE NOTICE '----> CONFLICT DETECTED with booking ID %', conflicting_booking.id;
    END IF;
  END LOOP;
  RAISE NOTICE '--- End Conflict Check ---';
  -- ### DEBUGGING LOGS END ###

  -- Original check remains to enforce the rule
  IF EXISTS (
    SELECT 1 FROM public.bookings 
    WHERE room_id = p_room_id 
    AND status NOT IN ('CANCELLED', 'EXTENDED', 'COMPLETED')
    AND (date_debut_prevue, date_fin_prevue) OVERLAPS (v_forced_start_date, v_forced_end_date)
  ) THEN
