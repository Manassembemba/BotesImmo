...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Conflit de réservation ou période de nettoyage bloquée détectés pour cette chambre.';
  END IF;
  ...
  INSERT INTO public.bookings (
    ...
    p_caution_encaissee,
    p_notes,
    CASE WHEN p_is_immediate_checkin THEN 'CONFIRMED' ELSE 'PENDING' END,
    NOW(),
    NOW()
  ...
  IF p_is_immediate_checkin THEN
    UPDATE public.rooms
    SET status = 'Occupé', updated_at = NOW()
    WHERE id = p_room_id;
  END IF;
  ...
  -- Calcul du montant total en équivalent USD
  ...
  IF v_final_amount >= p_prix_total THEN
    v_invoice_status := 'PAID';
  ELSIF v_final_amount > 0 THEN
  ...
  INSERT INTO public.invoices (
    ...
    JSONB_BUILD_ARRAY(
      JSONB_BUILD_OBJECT(
        'id', gen_random_uuid(),
        'description', 'Location Appartement - ' || v_room.numero || ' (' || v_room.type || ') - ' || v_nights || ' nuits',
        'quantity', v_nights,
    ...
    p_prix_total,
    'USD',
    COALESCE(p_notes, 'Facture générée automatiquement'),
