-- Migration: Update booking creation to use exact times
-- Date: 19 janvier 2026
-- Objectif: Supprimer la logique qui force les heures de début et de fin, pour utiliser les valeurs exactes (datetime) fournies par l'interface.
...
BEGIN
  -- This function is complex, and the full version from the previous state is used here.
  -- Only the relevant part is changed as per the plan.

  -- Get exchange rate (logic omitted for brevity, it's unchanged)
  ...
  -- ### MODIFICATION START ###
  -- Utiliser directement les dates et heures fournies par les paramètres
  v_forced_start_date := p_date_debut_prevue;
  v_forced_end_date := p_date_fin_prevue;
  -- ### MODIFICATION END ###
  ...
  -- 2. NORMALISER LES MONTANTS DE PAIEMENT
  ...
  -- 4. CRÉER LE PAIEMENT INITIAL SI NÉCESSAIRE
