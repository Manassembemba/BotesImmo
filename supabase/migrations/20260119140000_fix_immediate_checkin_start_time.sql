-- Migration: Fix start time for immediate check-ins
-- Date: 19 janvier 2026
-- Objectif: Corriger la logique pour que l'heure de début d'un check-in immédiat soit NOW() au lieu de minuit.
...
BEGIN
  ...
  -- Forcer l'heure de début
  IF p_is_immediate_checkin THEN
    -- Pour un check-in immédiat, l'heure de début est maintenant.
    v_forced_start_date := NOW();
  ELSE
    -- Pour une réservation future, l'heure de début est 12h00.
  ...
  INSERT INTO public.bookings (
    ...
    CASE WHEN p_is_immediate_checkin THEN v_forced_start_date ELSE NULL END, -- Utiliser v_forced_start_date pour check_in_reel
    p_prix_total,
