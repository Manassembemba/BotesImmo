-- Migration: Update Bookings RPC with Location Filter
-- Date: 2026-01-12
-- Description: Adds p_location_id parameter to get_bookings_with_financials to support Admin filtering.

DROP FUNCTION IF EXISTS public.get_bookings_with_financials(text,text[],timestamp with time zone,timestamp with time zone,integer,integer);
...
CREATE OR REPLACE FUNCTION public.get_bookings_with_financials(
  ...
  p_location_id uuid DEFAULT NULL::uuid -- NEW PARAMETER
)
RETURNS TABLE(
  id uuid,
  room_id uuid,
  ...
  room_number text, -- changed from varchar
  room_type text, -- changed from varchar
  location_id uuid,
  location_name text, -- changed from varchar
  tenant_nom text, -- changed from varchar
  tenant_prenom text, -- changed from varchar
  tenant_email text, -- changed from varchar
  tenant_telephone text, -- changed from varchar
  agent_email text, -- changed from varchar
  ...
  WITH filtered_bookings AS (
    ...
    WHERE 
      ...
      (
        ...
        v_is_admin 
        AND 
        -- If filter is provided, enforce it. If null, show all.
      ...
      (
        ...
        NOT v_is_admin 
        AND 
        r.location_id = v_user_location_id
        -- Agents ignore p_location_id (they can't switch)
      )
      -- SEARCH FILTERS
