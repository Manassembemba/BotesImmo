...
-- 4. RPC get_bookings_with_financials: Allow SERVICE_CLIENT global multi-site visibility
...
DECLARE
  ...
  v_is_global boolean;
BEGIN
  -- Check if user has global multi-site scope (ADMIN or SERVICE_CLIENT)
  v_is_global := public.has_role('ADMIN'::public.user_role, auth.uid())
              OR public.has_role('SERVICE_CLIENT'::public.user_role, auth.uid());
  ...
  WITH filtered_bookings AS (
    ...
    WHERE 
      (
        (v_is_global AND (p_location_id IS NULL OR r.location_id = p_location_id))
        OR 
        (NOT v_is_global AND r.location_id = v_user_location_id)
      )
