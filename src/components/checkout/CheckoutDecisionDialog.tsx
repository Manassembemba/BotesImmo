...
export function CheckoutDecisionDialog({ open, onOpenChange, booking, room }: CheckoutDecisionDialogProps) {
  ...
  const [paymentUSD, setPaymentUSD] = useState<number>(0);
  const [paymentCDF, setPaymentCDF] = useState<number>(0);

  const { data: exchangeRateData } = useExchangeRate(); // Récupérer le taux
  const currentRate = exchangeRateData?.usd_to_cdf || 2800;
  ...
  const extensionDetails = useMemo(() => {
    ...
    if (!isValid(currentEndDate) || !isValid(newEndDateObj) || newEndDateObj <= currentEndDate) {
      return { extraNights: 0, extraCost: 0, extraDiscount: 0, netExtensionCost: 0 };
    }
    ...
    const netExtensionCost = Math.max(0, extraCost - extraDiscount);

    return { extraNights, extraCost, extraDiscount, netExtensionCost };
  }, [newEndDate, booking, room, discountPerNight]);

  // Total encaissé immédiatement lors de la prolongation (en USD équivalent)
  const totalPaidInUSD = useMemo(() => {
    return (paymentUSD || 0) + ((paymentCDF || 0) / currentRate);
  }, [paymentUSD, paymentCDF, currentRate]);
  ...
  const handleExtendStay = async () => {
    ...
    try {
      await extendStay.mutateAsync({
        ...
        newTotalBookingPrice: finalPrice,
        extensionDiscountPerNight: discountPerNight,
        initialPaymentUSD: paymentUSD || 0,
        initialPaymentCDF: paymentCDF || 0,
      ...
      toast({
        ...
        description: `Le séjour a été prolongé jusqu'au ${format(extendedEndDate, 'dd/MM/yyyy')}${totalPaidInUSD > 0 ? ` avec un paiement de ${totalPaidInUSD.toFixed(2)}$` : ''}.`
      });
    } catch (error) {
      toast({
        variant: 'destructive',
        ...
        {mode === 'choice' ? (
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Réduction par nuit (Éditable pour ajuster la remise lors de la prolongation) */}
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    type="number"
                    min="0"
                    max={room.prix_base_nuit}
                    step="0.5"
                    value={discountPerNight}
                    onChange={(e) => setDiscountPerNight(Math.max(0, parseFloat(e.target.value) || 0))}
                    className="pl-8"
                    placeholder="0"
                  />
            ...
            {extensionDetails.extraNights > 0 && (
              <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-md text-sm space-y-1 border border-blue-100 dark:border-blue-800">
                <div className="flex justify-between font-medium text-blue-900 dark:text-blue-200">
                  ...
                  <span>+ {extensionDetails.netExtensionCost.toFixed(2)} $</span>
                </div>
              </div>
            )}
            ...
            {/* Section Encaissement immédiat (USD / FC) */}
            <div className="space-y-3 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="flex justify-between items-center">
                <Label className="font-semibold text-sm text-slate-800 dark:text-slate-200">
                  Encaissement immédiat de la prolongation
                </Label>
                {extensionDetails.netExtensionCost > 0 && (
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    className="h-7 text-xs text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 font-medium"
                    onClick={() => {
                      setPaymentUSD(extensionDetails.netExtensionCost);
                      setPaymentCDF(0);
                    }}
                  >
                    Régler 100% en USD
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <span className="text-xs font-medium text-slate-600 dark:text-slate-400">Montant en USD ($)</span>
                  <div className="relative">
                    <Input
                      type="number"
                      min="0"
                      ...
                      placeholder="0.00"
                      value={paymentUSD || ''}
                      onChange={(e) => setPaymentUSD(Math.max(0, parseFloat(e.target.value) || 0))}
                ...
                <div className="space-y-1">
                  <span className="text-xs font-medium text-slate-600 dark:text-slate-400">Montant en FC (CDF)</span>
                  <div className="relative">
                    <Input
                      type="number"
                      min="0"
                      ...
                      placeholder="0"
                      value={paymentCDF || ''}
                      onChange={(e) => setPaymentCDF(Math.max(0, parseFloat(e.target.value) || 0))}
                      className="pl-9 bg-white dark:bg-slate-800 font-semibold text-blue-600"
                    />
                    <span className="absolute left-2.5 top-2.5 text-xs font-bold text-blue-500">FC</span>
                  </div>
                </div>
              </div>

              {/* Résumé de l'encaissement et reste dû */}
              <div className="flex justify-between items-center text-xs pt-1 border-t border-slate-200 dark:border-slate-800 font-medium">
                <span className="text-slate-600 dark:text-slate-400">
                  Perçu : <strong className="text-emerald-600">{totalPaidInUSD.toFixed(2)} $</strong>
                </span>
                <span className="text-slate-600 dark:text-slate-400">
                  Reste dû extension : <strong className={extensionDetails.netExtensionCost - totalPaidInUSD > 0.01 ? "text-amber-600" : "text-emerald-600"}>
                    {Math.max(0, extensionDetails.netExtensionCost - totalPaidInUSD).toFixed(2)} $
                  </strong>
                </span>
              </div>
            </div>
